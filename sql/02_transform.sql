...
DROP TABLE IF EXISTS fato_emprestimo;
DROP TABLE IF EXISTS dim_historico_credito;
DROP TABLE IF EXISTS dim_perfil_emprestimo;
...
CREATE TABLE dim_perfil_emprestimo (
...
CREATE TABLE dim_historico_credito (
...
CREATE TABLE fato_emprestimo (
    ...
    id_perfil_emprestimo INTEGER NOT NULL REFERENCES dim_perfil_emprestimo (id_perfil_emprestimo),
    id_historico_credito INTEGER NOT NULL REFERENCES dim_historico_credito (id_historico_credito),
...
INSERT INTO dim_cliente (idade, renda_anual, posse_residencia, tempo_empregada)
SELECT DISTINCT
    person_age,
    person_income,
...
INSERT INTO dim_perfil_emprestimo (finalidade_emprestimo, classificacao_emprestimo)
SELECT DISTINCT
    loan_intent,
    loan_grade
FROM staging_credit_risk
...
INSERT INTO dim_historico_credito (inadimplencia_historica, duracao_historico_credito)
SELECT DISTINCT
